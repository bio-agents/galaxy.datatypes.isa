...
indeed it is the uploader agent that takes care of uploading and placing the raw data of the dataset
...
At the moment, a reasonable strategy to support ISA datatypes might be the following:
    ...
    * update the existing Galaxy uploader (or create a new uploader agent) to properly handle
...
to a Galaxy agent: it can be done using the `extra_files_path` property of the input parameter.

    Example of agent command:
    -----------------------
...
from isaagents import isatab  # depends on https://github.com/ISA-agents/isa-api/tree/py2_isaagents-lite 62fb5f0
